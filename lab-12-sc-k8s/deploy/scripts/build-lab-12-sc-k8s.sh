...
cd ../../lab-12-sc-k8s-service

mvn $proxyStr clean
mvn $proxyStr -P k8s package

docker rmi $registry/lab-12-sc-k8s-service:latest
docker build -t $registry/lab-12-sc-k8s-service:latest .
docker push $registry/lab-12-sc-k8s-service:latest
docker images | grep lab-12-sc-k8s-service
